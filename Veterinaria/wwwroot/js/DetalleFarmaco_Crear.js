...
$(document).ready(function () {
        ...
        success: function (data) {
            ...
            if (data.data != null) {
                $.each(data.data, function (i, item) {
                    ...
                    if (item.estado == true) {
                        $("<option>").attr({ "value": item.codLaboratorio }).text(item.razonSocial).appendTo("#cboLaboratorio");
                    }
                })
        ...
        success: function (data) {
            ...
            if (data.data != null) {
                $.each(data.data, function (i, item) {
                    ...
                    if (item.estado == true) {
                        $("<option>").attr({ "value": item.codProveedor }).text(item.razonSocial).appendTo("#cboProveedor");
                    }
                })
    ...
    //OBTENER ViaAdministeracion
    jQuery.ajax({
        url: "/ViaAdministracion/Obtener",
        type: "GET",
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) {
            ...
            if (data.data != null) {
                $.each(data.data, function (i, item) {
                    ...
                    if (item.estado == true) {
                        $("<option>").attr({ "value": item.codVia }).text(item.via).appendTo("#cboVia");
                    }
                })
                $("#cboVia").val($("#cboVia option:first").val());
            }
    ...
    tabladata = $('#tbdata').DataTable({
        "ajax": {
            "url": "/DetalleFarmaco/Obtener",
            "type": "GET",
            "datatype": "json"
        },
        "columns": [
            { "data": "codDetalleFarmaco" },
            { "data": "concentracion" },
            { "data": "fechaRegistro" },
            {
            ...
            {
                "data": "objviaAdministracion", render: function (data) {
                    return data.via
                }
            },
            ...
            {
                "data": "codDetalleFarmaco", "render": function (data, type, row, meta) {
...
function abrirPopUpForm(json) {
    ...
    if (json != null) {
        ...
        $("#txtid").val(json.codDetalleFarmaco);
        $("#txtConcentracion").val(json.concentracion);
        $("#txtFechaRegistro").val(json.fechaRegistro);
        $("#cboLaboratorio").val(json.codLaboratorio);
        $("#cboProveedor").val(json.codProveedor);
        $("#cboVia").val(json.codVia);
        $("#txtNombreComercial").val(json.nombreComercial);
        $("#txtNumeroLote").val(json.numeroLote);
...
function Guardar() {
    ...
    if ($("#form").valid()) {
        ...
        var request = {
            objeto: {
                codDetalleFarmaco: parseInt($("#txtid").val()),
                concentracion: $("#txtConcentracion").val(),
                fechaRegistro: $("#txtFechaRegistro").val(),
                codLaboratorio: $("#cboLaboratorio").val(),
...
function eliminar($id) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            jQuery.ajax({
                url: "/DetalleFarmaco/Eliminar" + "?id=" + $id,
                type: "GET",
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                success: function (data) {
